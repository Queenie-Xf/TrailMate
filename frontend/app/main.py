...
# 2. 导入所有功能视图 (修复了之前因函数名不匹配导致的 ImportError)
...
from app.views.groups import render_groups_page # 对应 groups.py 的新别名
from app.views.chat import render_chat_page     # 对应 chat.py 的新入口

# 3. 注入高级主题 CSS (保留你原本精美的 UI 布局)
...
def main() -> None:
    ...
    # --- 核心路由渲染：根据 view_mode 加载对应的 300 行模块 ---
    view = st.session_state.get("view_mode", "home")

    if view == "home":
        # 加载包含搜索、AI推荐和地图的 300 行 home.py 逻辑
        render_home_page(user)
    elif view == "friends":
        render_add_friend_page(user)
    elif view == "groups":
        render_groups_page(user) # 现在对应 groups.py 里的 render_create_group_page
    elif view == "chat":
        render_chat_page()       # 现在对应 chat.py 里的聊天渲染逻辑
